...
module.exports = {
    async getSeasonRanking(countryCode, seasonId, params = { limit: Infinity }) {
        if(!countryCode) throw new Error(`(brawlstars-api.js) "${countryCode}" id not a valid Two letter country code.`)
        else if(!seasonId) throw new Error(`(brawlstars-api.js) The season id "${seasonId}" is not a valid id.`)
    ...
    async getBrawlerRanking(countryCode, brawlerId, params = { limit: Infinity }) {
        if(!countryCode) throw new Error(`(brawlstars-api.js) "${countryCode}" id not a valid Two letter country code.`)
        else if(!brawlerId) throw new Error(`(brawlstars-api.js) The brawler id "${brawlerId}" is not a valid id.`)
        params.limit === Infinity && delete params.limit
